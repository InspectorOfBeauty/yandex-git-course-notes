# VCS
Определения:
- **VCS** - Vesion Control System - система контроля версий.
- **SCM** - Sourse Control Managment - система управления исходным кодом.
...
3. **анализировать изменения:** кто какие изменения вносит и как часто.<br>
...
- ~ - домашняя директория.
- все скрытые файлы помечаются точкой вначале.
...
Чтобы участники проекта знали, кто какие изменения вносил, нужно представитья: указать имя и почту (любые) с помощью команды **git config --global**. Не завимимо где сейчас находися, эта команда сработает в любом случае.Команды будут выглядеть следующим образом: <br>
```git config --global user.name "Ivan Ivanov"``` <br>
```git config -- blobal user.email ivanov.yndes.ru```


Все глобальные настройки гит хранит в файле .gitconfig в домашней директории, чтобы это проверить, можно прочесть его с помощью команды: <br>
```cat ~/.gitconfig``` <br>
...
Чтобы гит начал отслеживать изменения в проекте, папку с файлами нужно сделать ГИТ-репозиторием (хранилищем).
Для этого нужно сначала перейти в нее и затем выполнив команду: <br>
```git init``` <br>
...
Добавив файл в репозиторий, он помечаетя как **"неотслеживаемый"**. Состояние **untracked** значит, что ГИТ еще не хранит информацию о версиях этого файла и не может отследить, как он изменился. <br>
Зеленый цвет файла означает то, что файл отслеживается, но еще не сохранен. 


git add запоминает только состояние файла (контента). git add не сохраняет содержимое файлов в репозиторий. Само сохранение называют коммитом (совершать, фиксировать). Сделать коммит - сохранить текущую версию файла.
...
Определения:
- **Коммит** - одна из основных сущностей в ГИТе. Коммит гарантирует, что все изменения будут сохранены в истории и при желании к ним можно будет откатиться (аналогия: CTRL+Z для всего репозитория). 
- **root-commit** - самый первый коммит в ветке.


**Отличие git add и git commit:** <br>
git add сообщает ГИТу какие файлы нужно сохранить и какую их версию. <br>
git commit - само сохранение.


В сообщении после коммита можно встретить:
...
## Что такое GitHub?
**GitHub** - платформа для хранения проектов и совместной работы с использованием ГИТа. Это сайт, куда можно загружать свои проекты (гит-репозитории) и делиться ими. **hub** - узловая станция.
...
## Что такое SSH?
Команды:
- ```ls -la .ssh/``` - проверить наличие SSH-ключа. По-умолчанию он хранится в домашней дирктории в папка .ssh/
...
Один из распространенных протоколов - **SSH** - Secure Shell - безопасная оболочка. Он обеспечивает безопасность обмена данными в сети. С помощью этого протокола можно получить данные с удаленного компьютера или отправить их на него. Трафик шифруется, поэтому он безопасен.
...
- **ПУБЛИЧНЫЙ КЛЮЧ** - доступен всем и используется для шифрования данных. Они могут быть расшифрованы парным приватным ключом. <br>
Эти два ключа связаны и образуют пару. 


Таким образом SSH-ключ - это наш идентификатор в GitHub, как ключ от двери. Он позволяет нам получить доступ к удаленному Git-репозиторию.


Проверка наличия SSH-ключа. По-умолчанию он хранится в домашней дирктории в папка .ssh/ и можно проверить командой:
```ls -la .ssh/```
...
Инструкция:
1. Для генерации пары необходимо использовать программу ssh-keygen. Для этого необходимо открыть терминал и выполнить команду: **ssh-keygen -t ed22519 "accountEmail"** <br>
Если ошибка, скорее всего система не поддерживает алгоритм шифрования (ed22519). Поэтому выполняем другую команду: **ssh-keygen -t rsa -b 4096 -C "accountEmail"** <br>
В том случае, если все сделано правильно, отобразится сообщение: <br>
*"Generating public/private rsa key pair."*
2. После чего нужно указать место хранения ключей. Нажав enter, по-умолчанию сделается домашний каталог место для их хранения.  
...
```ls -a ~/.ssh``` <br>
Должно вывестись два файла, один с расширением .pub, другой - без. Тот, что .pub - публичный, им можно делиться. Тот, что без расширния - приватный и не передается никому.
...
и скопировать вручную в буфер (ПКМ).
2. Перейти в GitHub в **Settings** (меню акк). 
...
Если это было первое подключение к серверу, появится сообщение:
*"The authenticity of host 'github.com ..."* <br>
Если не первое, то: <br>
*"Hi UserName! You've successfully authenticated, but GitHub does not provide shell access."*


## Связыание локального репозитория с удаленным
Инструкция:
1. Для начала нужно перейти на страницу удаленного репозитория, выбрать тип SSH и скопировать URL.
2. Затем, чтобы привзяать удаленный репозиторий к локальному, нужно перейти в директорию локального репозитоия и выполнить команду:
...
В первый раз эту команду нужно вызывать с флагом ```-u``` и параметрами: имя удаленного репозитория (origin) и название текущей ветки (main/master). Флаг ```-u```` свзяет локальую ветку с одноименной удаленной. В дальнейшем флаг можно опустить.
...
- для чего,
- какие решает задачи;
2. Технологии, что встречаются в проекте. Чем он отличается от других.
3. Документация проекта - подробная инструкция, что представляет из себя проект.
4. Планы проекта.


README можно отображать с помщоью удобной разметки. Markdown - специальный язык разметки. 
...
Новый параграф: <br>
два символа переноса (два enter) <br><br>
...
```[Текст ссылки] (url)``` <br><br>

Оформление кода с помощью грависов: <br>
``` ```код``` ```
